...
async function callApi(
  ...
  if (!res.ok) {
    return { status: res.status, error: (data as Record<string, string>).error || `HTTP ${res.status}` };
  }
...
server.tool(
  ...
  async () => {
    ...
    if (res.error) return textResult({ error: res.error });
...
server.tool(
  ...
  async ({ service }) => {
    ...
    if (service && data?.services) {
      ...
      if (!svc) return textResult({ error: `Unknown service: ${service}` });
...
server.tool(
  ...
  async ({ audio_url, language, diarize, punctuate }) => {
    const body: Record<string, unknown> = { audio_url };
    if (language) body.language = language;
    if (diarize) body.diarize = diarize;
...
server.tool(
  ...
  async ({ chain, address, min_liquidity, min_volume_24h }) => {
    ...
    if (min_liquidity) body.min_liquidity = min_liquidity;
    if (min_volume_24h) body.min_volume_24h = min_volume_24h;
...
server.tool(
  ...
  async ({ origin, destination, departureDate, adults, returnDate, max, nonStop, currencyCode }) => {
    const params: Record<string, unknown> = { origin, destination, departureDate };
...
server.tool(
  ...
  async ({ cityCode, checkInDate, checkOutDate, adults, roomQuantity, priceRange, currency }) => {
    const params: Record<string, unknown> = { cityCode, checkInDate, checkOutDate };
    if (adults) params.adults = adults;
...
server.tool(
  ...
  async ({ origin, destination, departureDate, oneWay }) => {
    const params: Record<string, unknown> = { origin, destination };
    if (departureDate) params.departureDate = departureDate;
    if (oneWay) params.oneWay = oneWay;
