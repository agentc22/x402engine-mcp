...
function parseNetwork(value: string | undefined): PaymentNetwork {
...
export function usdToAtomicUnits(value: string): bigint {
...
function isKnownUsdc(requirement: PaymentRequirements): boolean {
...
export function selectPaymentRequirement(
  ...
  config: Pick<PaymentConfig, "network" | "maxPaymentUsd">,
): PaymentRequirements {
  ...
  if (affordable.length === 0) {
    ...
    throw new Error(
      `Payment of $${(Number(cheapest.amount) / 1_000_000).toFixed(6)} exceeds X402_MAX_PAYMENT_USD=${config.maxPaymentUsd}`,
    );
  }
...
function parseEvmKey(value: string): `0x${string}` {
...
function parseSolanaKey(value: string): Uint8Array {
...
export async function createPaymentFetch(
  config: PaymentConfig = loadPaymentConfig(),
  ...
  const client = new x402Client((_version, requirements) =>
    selectPaymentRequirement(requirements, config),
  );
